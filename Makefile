...
# The Claude Code server is driven over a fake `claude` that speaks the CLI's
# stream-json shapes.  The claim under test: `send` never waits, and the voice
...
sabotage:
	...
	for suite in tool_loop_test mcp_test mcp_files_test mcp_web_test mcp_claude_test mcp_codex_test events_test turn_control_test approvals_test; do \
