...
| Silence between the question and the first word | The bridge streams prose as `delta` events and the page speaks the first complete sentence at once (`speakStreamed` in `web/chat.js`); if the bridge has no tools attached the page does not stream and waits for the whole answer. `tests/tool_loop_test.py` (streaming cases) and `voice_tts_browser` (turn 3) are the gates. |
